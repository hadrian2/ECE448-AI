...
def minimax(side, board, flags, depth):
    ...
    d = {}
    if depth == 0 :
        return evaluate(board), l, d
    else:
        vd = {}
        ld = {}
        for move in generateMoves(side,board,flags):
            nside,nboard,nflags = makeMove(side, board, move[0], move[1], flags, move[2])
            vd[encode(*move)],ld[encode(*move)],d[encode(*move)] = minimax(nside,nboard,nflags,depth-1)
        if side == False:
            choose = max(vd,key=vd.get)
        else:
            choose = min(vd,key=vd.get)
        c = decode(choose)
        l = [c]+ld[choose]
        return vd[choose],l,d
...
def alphabeta(side, board, flags, depth, alpha=-math.inf, beta=math.inf):
    ...
    if depth == 0 :
        ...
        for move in generateMoves(side,board,flags):
            nside,nboard,nflags = makeMove(side, board, move[0], move[1], flags, move[2])
            vd[encode(*move)],ld[encode(*move)],d[encode(*move)]= alphabeta(nside,nboard,nflags,depth-1,alpha,beta)
            if side == False:
                choose = max(vd,key=vd.get)
                if vd[choose] > alpha:
                    alpha = vd[choose]
